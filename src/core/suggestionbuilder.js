/*
 * Avro Phonetic engine -- web port (chunk 9: suggestion builder)
 * Derived from ibus-avro (jsAvroPhonetic), (C) OmicronLab.
 * Original license: Mozilla Public License 2.0 (https://mozilla.org/MPL/2.0/)
 * This chunk: GJS/IBus/GTK/Gio dependencies removed; candidate-selection
 * persistence moved from a GIO file (~/.candidate-selections.json) to
 * localStorage, since a browser context has no home directory.
 */
Avro.SuggestionBuilder = function SuggestionBuilder(){
    this._init();
}

Avro.SuggestionBuilder.prototype = {
    
    _init: function(){
        this._dbSearch = new Avro.DBSearch();
    ...
    _getClassicPhonetic: function(banglish){
        return Avro.Utf8.decode(Avro.Phonetic.parse(banglish));
    },
    
    
    _correctCase:function (banglish){
        return Avro.Phonetic.fixString(banglish);
    },
    ...
    _getAutocorrect: function(word, splitWord){
        ...
        if(Avro.Data.autocorrect[word]){
            // [smiley rule]
            if (Avro.Data.autocorrect[word] == word){
                corrected['corrected'] = word;
                corrected['exact'] = true;
            } else {
                corrected['corrected'] = this._getClassicPhonetic(Avro.Data.autocorrect[word]);
            ...
            if(Avro.Data.autocorrect[correctedMiddle]){
                corrected['corrected'] = this._getClassicPhonetic(Avro.Data.autocorrect[correctedMiddle]);
                corrected['exact'] = false;
            }
        }
    ...
    _sortByPhoneticRelevance: function (phonetic, dictSuggestion){
        ...
        sortedSuggestion.sort(function(a, b){
            var da = Avro.Levenshtein.distance(phonetic, a);
            var db = Avro.Levenshtein.distance(phonetic, b);
    ...
    _addSuffix: function(splitWord){
        ...
        if (len >= 2){
            for (var j = 1; j <= len; j++){
                ...
                var suffix = Avro.Data.suffix[testSuffix];
                if (suffix){
    ...
    _storageKey: '.candidate-selections.json',

    // Replaces the original GIO file-based persistence (~/.candidate-selections.json)
    // with localStorage, since a web/userscript context has no home directory.
    _loadCandidateSelectionsFromFile: function(){
        try {
            var json = (typeof localStorage !== 'undefined') ? localStorage.getItem(this._storageKey) : null;
            this._candidateSelections = json ? (JSON.parse(json) || {}) : {};
        } catch (e){
           this._candidateSelections = {};
    ...
    _saveCandidateSelectionsToFile: function(){
        try {
            if (typeof localStorage !== 'undefined') {
                localStorage.setItem(this._storageKey, JSON.stringify(this._candidateSelections));
            }
        } catch (e) {
